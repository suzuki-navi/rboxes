...
def main():
    ...
    parser.add_argument('-d', '--directory', type=str, help='Output directory for HTML files')
    ...
    if args.source_path and os.path.isdir(args.source_path):
        if not args.directory:
            parser.error("Output directory (-d) is required when input is a directory")
        process_directory(args.source_path, args.directory, args.force)
    else:
        to_html(args.source_path, args.directory, args.force)
...
def to_html(source_path, output_path=None, force_overwrite=False):
    if source_path is None:
        file_binary_content = sys.stdin.buffer.read()
...
def markdown_to_html(markdown_text: str) -> str:
    ...
    javascript_template_path = os.path.join(os.path.dirname(__file__), 'mdlib.js')
    with open(javascript_template_path, 'r', encoding='utf-8') as f:
        javascript_template = f.read()

    template = template.replace('{script}', javascript_template)
